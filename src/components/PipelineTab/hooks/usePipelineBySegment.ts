 ...
 */
const calculateAllocationPortions = (opp: Record<string, any>, baseRevenue: number) => {
 ...
 */
const createSegmentGroup = (name: string, extra: Record<string, any> = {}) => ({
  name,
  early: 0,
  earlyAllocated: 0,
  earlyNonAllocated: 0,
 ...
 */
export const usePipelineBySegment = (filteredOpportunities: Record<string, any>[], showNetRevenue: boolean) => {
  /**
  ...
  const stackedSegmentData = useMemo(() => {
    ...
    filteredOpportunities.forEach((opp) => {
      const segment = opp.subSegmentCode || "Uncategorized";
  ...
  const getSubSegmentsBySegment = useCallback(
    (segmentCode: string) => {
      ...
      filteredOpportunities.forEach((opp) => {
        if (opp.subSegmentCode !== segmentCode) return;

        const subSegment = opp.subSegment || "Uncategorized";

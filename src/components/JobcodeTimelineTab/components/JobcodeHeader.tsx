...
const JobcodeHeader = React.memo(({ selectedJobcode, opportunityStreams = [] }: any) => {
  ...
  const avgDuration = useMemo(() => {
    ...
    const durations = opportunityStreams
      .filter((s: any) => s.firstDate && s.lastDate)
      .map((s: any) => s.lastDate - s.firstDate);
    ...
    <Box sx={{ mb: 3 }}>
      ...
      {opportunityStreams.length > 1 && (
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
          {opportunityStreams.map((stream: any, index: number) => {

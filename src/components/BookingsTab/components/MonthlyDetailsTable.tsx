  ...
  ({
    ...
    const yearColors = useMemo(() => {
      ...
      if (Object.keys(parentYearColors).length > 0) {
        ...
        displayYears.forEach((year: number) => {
          colors[year] = parentYearColors[year]?.bar || parentYearColors[year] || brand.primaryDark;
        });
        return colors;
      }
      ...
      displayYears.forEach((year: number, index: number) => {
    ...
    const maxMonthlyValue = useMemo(() => {
      ...
      let max = 0;
      cumulativeData.forEach((monthData: any) => {
        displayYears.forEach((year: number) => {
     ...
     */
    const formatVariation = (variation: number | null): string | null => {
      ...
      >
        ...
        <Collapse in={showDetailsTable}>
          <Box sx={{ px: 1.5, pb: 2.5, pt: 1 }}>
            ...
            >
              {cumulativeData.map((monthData: any, index: number) => {
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      border: `1px solid ${
                        isCurrent ? alpha(theme.palette.info.main, 0.4) : alpha(theme.palette.divider, 0.2)
                      }`,
                      transition:
                        ...
                        {displayYears.map((year, yearIndex) => {
                          ...
                          return (
                            <Box key={year} sx={{ mb: variationText && previousYear ? 0.75 : 0 }}>
                              ...
                              >
                                {formatCurrencyOrDash(cumulativeValue)}
                              </Typography>
                      ...
                      >
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.15 }}>
                          {displayYears.map((year) => (
                            ...
                            >
                              {formatCurrencyOrDash(monthData[`${year}_io_cumulative`] || 0)}
                            </Typography>
                          ))}
                        </Box>
                      </Box>

...
export const animations = {
  ...
  cardEntrance: (delay: number = 0) => ({
    animation: `fadeInUp 0.4s ${easing.elegant} ${delay}ms both`,
    ...keyframes.fadeInUp,
  }),

  // List item entrance (subtler, faster — for dense lists)
  listEntrance: (delay: number = 0) => ({
    animation: `fadeInUp 0.3s ${easing.elegant} ${delay}ms both`,
    ...keyframes.fadeInUp,
  }),
...
export const glassmorphism = (blur: number = 10, alphaVal: number = 0.7, dark: boolean = false) => ({
...
// Neumorphism Effect
export const neumorphism = (theme: Theme, pressed: boolean = false) => ({
  ...
  transition: `box-shadow ${timing.fast} ${easing.standard}`,
});
...
export const staggerChildren = (baseDelay: number = 0, increment: number = 100) => {
  return (index: number) => animations.cardEntrance(baseDelay + index * increment);
};

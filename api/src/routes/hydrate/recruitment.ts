...
router.get("/recruitment", (_req: Request, res: Response) => {
  try {
    const rows = db
      .prepare(
        `SELECT id, firstName, lastName, email, phone, status, poste, gradeBucket,
         ...
         FROM hr_candidates ORDER BY creationDate DESC`
      )
    ...
    const appRows = db
      .prepare("SELECT candidateId, jobPosting, segment, offering FROM hr_applications ORDER BY candidateId")
    ...
    const statusCounts = db.prepare("SELECT status, COUNT(*) as count FROM hr_candidates GROUP BY status").all() as {
      status: string;
      count: number;
    }[];

    const gradeBucketCounts = db
      .prepare("SELECT gradeBucket, status, COUNT(*) as count FROM hr_candidates GROUP BY gradeBucket, status")
    ...
    const segmentCounts = db
      .prepare(
        "SELECT segment, COUNT(DISTINCT candidateId) as count FROM hr_applications WHERE segment IS NOT NULL GROUP BY segment ORDER BY count DESC"
      )
      .all() as { segment: string; count: number }[];

    const offeringCounts = db
      .prepare(
        "SELECT offering, COUNT(DISTINCT candidateId) as count FROM hr_applications WHERE offering IS NOT NULL GROUP BY offering ORDER BY count DESC"
      )

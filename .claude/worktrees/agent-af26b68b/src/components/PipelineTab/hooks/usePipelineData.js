 ...
 */
export const usePipelineData = (data, loading, showNetRevenue, getDateFilteredData) => {
  // State management
  ...
  const [drillDownOffering, setDrillDownOffering] = useState(null);
  const [serviceLineFilteredData, setServiceLineFilteredData] = useState([]);
  ...
  const handleChartClick = useCallback(
    (chartEvent, isDrillDownMode = false) => {
      ...
      if (clickedItem.status) {
        ...
        if (isDrillDownMode && clickedItem.isOffering) {
          ...
          filtered = filteredOpportunities
            .filter(
              (opp) =>
                (opp["Service Line 1"] === drillDownServiceLine && opp["Service Offering 1"] === clickedItem.name) ||
                (opp["Service Line 2"] === drillDownServiceLine && opp["Service Offering 2"] === clickedItem.name) ||
                (opp["Service Line 3"] === drillDownServiceLine && opp["Service Offering 3"] === clickedItem.name)
            )
            .map((opp) => {
              // Calculate the percentage allocated to the clicked offering
              let allocatedPercentage = 0;
              if (opp["Service Line 1"] === drillDownServiceLine && opp["Service Offering 1"] === clickedItem.name) {
                allocatedPercentage += opp["Service Offering 1 %"] || 0;
              }
              if (opp["Service Line 2"] === drillDownServiceLine && opp["Service Offering 2"] === clickedItem.name) {
                allocatedPercentage += opp["Service Offering 2 %"] || 0;
              }
              if (opp["Service Line 3"] === drillDownServiceLine && opp["Service Offering 3"] === clickedItem.name) {
                allocatedPercentage += opp["Service Offering 3 %"] || 0;
              }

              // Calculate allocated revenue based on the percentage
              const allocationFactor = allocatedPercentage / 100;
              const allocatedGrossRevenue = (opp["Gross Revenue"] || 0) * allocationFactor;
              const allocatedNetRevenue = (opp["Net Revenue"] || 0) * allocationFactor;

              return {
                ...opp,
                "Allocated Service Line": drillDownServiceLine,
                "Allocated Offering": clickedItem.name,
                "Is Allocated": true,
                "Allocated Gross Revenue": allocatedGrossRevenue,
                "Allocated Net Revenue": allocatedNetRevenue,
                "Allocation Percentage": allocatedPercentage,
              };
            });
          ...
          filtered = sourceData
            .filter(
              (opp) =>
                opp["Service Line 1"] === clickedItem.name ||
                opp["Service Line 2"] === clickedItem.name ||
                opp["Service Line 3"] === clickedItem.name
            )
            .map((opp) => {
              ...
              if (opp["Service Line 2"] === clickedItem.name) {
                allocatedPercentage += opp["Service Offering 2 %"] || 0;
              }
              if (opp["Service Line 3"] === clickedItem.name) {
                allocatedPercentage += opp["Service Offering 3 %"] || 0;
              }
              ...
              return {
                ...opp,
                "Allocated Service Line": clickedItem.name,
                "Is Allocated": true,
                "Allocated Gross Revenue": allocatedGrossRevenue,
                "Allocated Net Revenue": allocatedNetRevenue,
                "Allocation Percentage": allocatedPercentage,

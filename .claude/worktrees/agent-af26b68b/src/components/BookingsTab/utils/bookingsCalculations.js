 ...
 */
const getOpportunitySource = (opp) => {
 ...
 */
const calculateSourceBreakdown = (opps, showNetRevenue) => {
  const breakdown = {
  ...
  opps.forEach((opp) => {
    ...
    breakdown[source].value += revenue;
    breakdown[source].count += 1;
    breakdown[source].io += ioRevenue;
    breakdown[source].allocated += allocatedRevenue;
  });
 ...
 * PERFORMANCE: Cette fonction est lourde (250+ lignes), elle DOIT être mémoïsée avec useMemo
 */
export function calculateCumulativeTotals(
  ...
  const result = bookingsData.map((monthData, index) => {
    ...
    yearsParam.forEach((year) => {
      ...
      const monthlyAllocatedRevenue = monthlyOpps.reduce((sum, opp) => {
        if (opp["Is Allocated"] && opp["Allocated Gross Revenue"]) {
          return sum + (showNetRevenue ? opp["Allocated Net Revenue"] || 0 : opp["Allocated Gross Revenue"]);
        }
  ...
  yearsParam.forEach((year) => {
    result.forEach((monthData, index) => {
      ...
      const cumulativeAllocated = cumulativeOpps.reduce((sum, opp) => {
        if (opp["Is Allocated"] && opp["Allocated Gross Revenue"]) {
          return sum + (showNetRevenue ? opp["Allocated Net Revenue"] || 0 : opp["Allocated Gross Revenue"]);
        }
        return sum + (showNetRevenue ? opp["Net Revenue"] || 0 : opp["Gross Revenue"] || 0);
